...
import { FaClipboardCheck } from 'react-icons/fa'; // Import icon
...
const AttendancePage = () => {
    ...
    // Dummy data structure for initial state or fallback
    ...
    useEffect(() => {
        const fetchAttendance = async () => {
            try {
                ...
                setError("Failed to load attendance data. Please ensure the backend is running on port 3001.");
                setStudentAttendance(dummyAttendance); // Fallback to dummy data
            } finally {
                setLoading(false);
            }
        };
    ...
    if (error && studentAttendance.length === 0) {
        return (
