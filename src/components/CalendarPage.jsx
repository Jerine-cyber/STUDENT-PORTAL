...
const CalendarPage = () => {
  ...
  // Dummy data for Karpagam College academic calendar. (Data remains unchanged)
      ...
      <div className="w-full max-w-4xl p-8 transition-all duration-500 transform bg-white shadow-3xl rounded-3xl backdrop-filter backdrop-blur-sm">
        ...
        <div className="space-y-8">
          {academicCalendar.map((monthData) => (
            <div 
              key={monthData.month} 
              // 3D Effect on Month Block: Lifted shadow, subtle transform on hover
            ...
            >
              ...
              <ul className="p-4 space-y-3">
                {monthData.events.map((event, index) => (
                  <li 
                    key={index} 
                    // 3D Effect on Event Item: Sharp borders, hover background
